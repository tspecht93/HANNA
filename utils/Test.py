class Utils:         
    def predict(self, embedding_matrix, scaler, model, device): # Function to predict ln_gamma values
        # Transform input into the correct shape
        X_test_exp_processed = self.preprocess_input(embedding_matrix) # Shape: [num_samples, 2, 2+ BERT_Embedding]
        ...
        T_Test_exp, x_Test_exp, FP_Test_exp = self.split_and_reshape_input(X_test_exp_scaled) # Shapes: [num_samples], [num_samples, 1], [num_samples, 2, BERT_Embedding]
        # Convert to tensors
    ...
    def preprocess_input(self, embedding_matrix, Embedding_BERT=384, num_components=2):
    ...
    def split_and_reshape_input(self, input_array):
    ...
    def get_smiles_embedding(self, smiles, model_version='DeepChem/ChemBERTa-77M-MTR', max_length=128):
    ...
    def create_embedding_matrix(self, smiles1, smiles2, T, x1_values=None):
        # Canonicalize the SMILES
        smiles1 = self.canonicalize_smiles(smiles1)
        smiles2 = self.canonicalize_smiles(smiles2)
        
        # Get embeddings for both SMILES
        emb1 = self.get_smiles_embedding(smiles1).flatten()
        emb2 = self.get_smiles_embedding(smiles2).flatten()
